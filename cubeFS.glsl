...
void main(void) {
    ...
    vec3 fragmentColor = vec3(diffuseCol.r * c, diffuseCol.g * c, diffuseCol.b) * dlColor * clamp(dot(n, l), 0.f, 1.f) // directional light diffuse component
                       + vec3(.5f, .5f, .5f) * pow(clamp(dot(e, r), 0.f, 1.f), 16.f) // directional light specular component
                       + vec3(diffuseCol.r * c, diffuseCol.g * c, diffuseCol.b) * plColor * clamp(dot(n, lPointLight), 0.f, 1.f) / (plDist * plDist) // point light diffuse component
                       + vec3(.2f, .2f, .2f) * pow(clamp(dot(e, rPointLight), 0.f, 1.f), 16.f) / (plDist * plDist) // point light specular component
                       + ambientCol;
