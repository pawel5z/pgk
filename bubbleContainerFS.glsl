...
void main(void) {
    ...
    vec3 fragmentColor = diffuseCol * dlColor * clamp(dot(n, l), 0.f, 1.f) // directional light diffuse component
                       ...
                       + diffuseCol * dlColor * clamp(dot(n, lPointLight), 0.f, 1.f) / (plDist * plDist) // point light diffuse component
                       + vec3(1.f) * 10.f * pow(clamp(dot(e, rPointLight), 0.f, 1.f), 64.f) * float(dot(lPointLight, n) > 0.f) / (plDist * plDist) // point light specular component
                       + ambientCol;
